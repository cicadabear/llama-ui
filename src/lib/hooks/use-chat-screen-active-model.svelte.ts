...
export function useChatScreenActiveModel() {
	...
	let lastFetchedModel: string | null = $state(null);

	$effect(() => {
		if (activeModelId && activeModelId !== lastFetchedModel) {
			lastFetchedModel = activeModelId;

			const cached = modelsStore.props.getModelProps(activeModelId);

			if (!cached) {
				modelsStore.props.fetchModelProps(activeModelId).then((props) => {
					if (props) modelPropsVersion++;
				});
			}
		}
	});
