...
export class ModelPropsManager {
	...
	private failedModelIds = new Set<string>();
	...
	async fetchModelProps(modelId: string): Promise<ApiLlamaCppServerProps | null> {
		...
		// The /props?model= endpoint is absent on non-llama.cpp backends (e.g.
		// vllm). Once it 404s for a model, remember that and stop retrying so
		// reactive callers cannot hammer the endpoint in a loop.
		if (this.failedModelIds.has(modelId)) return null;
		...
		try {
			...
			if (error instanceof ApiError && error.status === 404) {
				this.failedModelIds.add(modelId);
			}
