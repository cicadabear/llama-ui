	...
	async fetch({ background = false }: { background?: boolean } = {}): Promise<void> {
		...
		const fetchPromise = (async () => {
			try {
				...
				// A configured third-party OpenAI-compatible backend (e.g. vllm)
				// does not expose /props. A 404 there is expected, not a failure:
				// fall back to a basic single-model (MODEL) endpoint so the user
				// can still chat and measure token speed.
				if (status === 404 && getApiBase()) {
					this.props = null;
					this.error = null;
					this.status = null;
					this.detectRole({} as ApiLlamaCppServerProps);
					return;
				}
