...
export class ChatService {
	...
	static async handleStreamResponse(
		...
		// --- client-side speed measurement (backend-agnostic) ---
		// llama.cpp servers embed per-chunk timings in the stream; a plain
		// OpenAI-compatible endpoint (e.g. vllm) does not, so we measure TTFT
		// and generation throughput directly from the client clock.
		...
		try {
					...
					for (const line of lines) {
						...
						if (line.startsWith(SSE_DATA_PREFIX)) {
							...
							try {
								...
								// vllm 0.27.x streams the model's "thinking" in
								// delta.reasoning; OpenAI extensions use reasoning_content
			...
			if (streamFinished) {
				...
				// client-side speed sample (works against any endpoint)
	...
	static async sendMessage(
		...
		if (stream) {
			// ask the endpoint to include usage in the final chunk (vllm/OpenAI
			// support this; llama.cpp ignores it) so we can count tokens accurately
			(requestBody as { stream_options?: { include_usage?: boolean } }).stream_options = { include_usage: true };
		}
